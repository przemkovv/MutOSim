...
prepare_scenario_global_A(const Config::Topology &config, Intensity A)
{
  ...
  {
    auto &g = topology.add_group(std::make_unique<Simulation::Group>(name, group.capacities[0]));
...
prepare_scenario_local_group_A(const Config::Topology &config, Intensity A)
{
  ...
  for (const auto &[name, config_group] : config.groups)
  {
    auto &group = topology.add_group(
        std::make_unique<Simulation::Group>(name, config_group.capacities[0], config_group.layer));
    ...
    for (const auto &tcs : config_group.traffic_classess_settings)
    {
      for (const auto &cr : tcs.second.compression_ratios)
      {
        group.add_compression_ratio(tcs.first, cr.threshold, cr.size, cr.intensity_factor);
      }
      if (tcs.second.block)
      {
  ...
  for (const auto &source : config.sources)
  {
    weights_sum_per_group[source.attached] += config.traffic_classes.at(source.tc_id).weight;
  }
  ...
  for (const auto &source : config.sources)
  {
    ...
    const auto &tc = topology.add_traffic_class(
        cfg_tc.id, offered_intensity, cfg_tc.serve_intensity, cfg_tc.size, cfg_tc.max_path_length);

    topology.add_source(Simulation::create_stream(source.type, source, tc));
    topology.attach_source_to_group(source.name, source.attached);

    traffic_intensity += tc.source_intensity / tc.serve_intensity * tc.size / total_capacity;
  }
